...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--investor-chunk-trading-days", type=int)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.investor_chunk_trading_days is not None:
        if args.dataset != "investor" or args.investor_chunk_trading_days < 1:
            raise SystemExit("--investor-chunk-trading-days requires dataset=investor and a positive value")
    try:
    ...
    result = run_short_selling_batch(
        ...
        throttle=throttle,
        investor_max_trading_dates=args.investor_chunk_trading_days,
    )
