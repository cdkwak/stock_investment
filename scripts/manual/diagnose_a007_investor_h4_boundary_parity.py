...
from scripts.manual import a007_investor_h4_boundary_parity_support as support
from scripts.manual import diagnose_a007_investor_range as base
from scripts.manual.pykrx_short_selling_pilot_support import AppendOnlyLedger,PilotStopped,assert_no_credentials,d_owned_run_lock,redact,utc_now
...
def run(*,env_file=ROOT/".env",project_root=ROOT,session_getter=None,execute=None):
 ...
 try:
  with d_owned_run_lock((project_root/"data/state/d_owned_krx_short_selling.lock"),run_id=rid):
   with base.HttpCapture(ledger=led,run_dir=rd,project_root=project_root,credential_values=creds,run_id=rid,diagnostic_support=support,expected_dates=dates) as cap:
    ...
    if execute: execute(transport)
    else:
     for scope in support.SCOPES: transport.post("https://data.krx.co.kr/comm/bldAttendant/getJsonData.cmd",data={"bld":support.BUSINESS_BLD,**{k:str(v) for k,v in scope.items() if k!="name"}})
    responses=cap.take_exact_responses()
    if cap.raw_count!=8: raise PilotStopped(f"RAW_REQUEST_COUNT_MISMATCH:{cap.raw_count}/8")
    classification,parts=support.classify_responses([r.content for r in responses],dates); led.append("DIAGNOSTIC_PASSED",classification=classification,raw_http_requests=8,business_requests=3,parts=[r.classification for r in parts]); result={"status":"PASS","classification":classification,"run_dir":str(rd),"raw_http_requests":8,"business_requests":3}
