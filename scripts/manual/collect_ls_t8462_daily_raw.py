...
try:
    ...
    from scripts.manual.ls_derivatives_raw_backfill import (
...
def _prior_attempt_exists(root: Path, market_date: str) -> bool:
    landing = root / "data/landing/ls_openapi/t8462_daily_raw" / market_date
    return landing.exists() and any(path.name == "checkpoint.json" for path in landing.glob("*/checkpoint.json"))
...
def main() -> int:
    ...
    if not args.confirm_live_daily_raw:
        ...
        return 2
    if _prior_attempt_exists(args.root, market_date):
        ...
        if not auth_ok:
            ...
            for sequence, scope in enumerate(plan, start=1):
                ...
                provenance = {
                    ...
                    "semantic_status": {
                        "sv": "SIGNED_NET_QUANTITY", "sa": "UNIT_INFERRED_MULTI_DATE_MATCH",
                        "U": "SESSION_INFERRED_ALL", "D": "SESSION_UNRESOLVED", "N": "SESSION_UNRESOLVED",
                        "option_u_historical_difference": "OPTION_SPECIFIC_SEMANTICS",
                    },
