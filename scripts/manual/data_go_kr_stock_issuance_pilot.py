...
class CaptureSession:
    ...
    def __init__(self, delegate, run_root: Path, service_key: str) -> None:
...
def _validate_items(items: tuple[dict[str, object], ...]) -> dict[str, object]:
    if len(items) != EXPECTED_TOTAL:
        raise PilotError("returned item count differs from retained guide expectation")
...
def run_pilot(project_root: Path, *, delegate=None) -> dict[str, object]:
    ...
    capture = CaptureSession(delegate or __import__("requests"), run_root, service_key)
