...
from stock_data.contracts.bok_ecos_market_rates import BOK_ECOS_MARKET_RATE_CONTRACTS
...
from stock_data.contracts.kr_etf_investor_flow import KR_ETF_INVESTOR_FLOW_DAILY
from stock_data.contracts.us_option_pcr import CBOE_DAILY_PCR_DAILY
from stock_data.contracts.kbsec_transactions import KBSEC_TRANSACTIONS_DAILY
...
CONTRACTS = {contract.name: contract for contract in (
    ...
    *BOK_ECOS_MARKET_RATE_CONTRACTS,
    *STOCK_ISSUANCE_OBSERVATION_CONTRACTS,
    *KRX_DERIVATIVES_INVESTOR_CONTRACTS,
    *LS_T8428_CONTRACTS,
    *KR_ETF_CONTRACTS,
...
if len(CONTRACTS) != 32 + len(DATA_V1_CONTRACTS) + len(KR_DERIVATIVE_CONTRACTS) + len(KBSEC_SNAPSHOT_CONTRACTS) + len(LEGACY_KOSPI200_CONTRACTS) + len(LEGACY_MARKET_INVESTOR_CONTRACTS) + len(INVESTOR_BRIDGE_CONTRACTS) + len(DIVIDEND_OBSERVATION_CONTRACTS) + len(KOSPI200_DERIVATIVES_BRIDGE_CONTRACTS) + len(TOSSINVEST_HISTORICAL_CONTRACTS) + len(BOK_ECOS_TREASURY_CONTRACTS) + len(BOK_ECOS_FX_CONTRACTS) + len(BOK_ECOS_MARKET_RATE_CONTRACTS) + len(STOCK_ISSUANCE_OBSERVATION_CONTRACTS) + len(KRX_DERIVATIVES_INVESTOR_CONTRACTS) + len(LS_T8428_CONTRACTS) + len(KOSPI200_CONSTITUENT_BREADTH_CONTRACTS) + len(KR_ETF_CONTRACTS):
