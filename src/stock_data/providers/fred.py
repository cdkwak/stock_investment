...
def fetch_series(series_id: str, start: date | None = None, *, session=requests) -> pd.DataFrame:
    params = {"id": series_id}
...
def fetch_dataset(series_ids: tuple[str, ...], start: date | None = None) -> pd.DataFrame:
    frames = [fetch_series(series_id, start) for series_id in series_ids]
    result = frames[0]
...
def collect_fred(root: Path, *, start: date | None = None) -> dict[str, pd.DataFrame]:
    ...
    for contract,series in configs:
        ...
        incoming=fetch_dataset(series,request_start)
        if existing is not None:
