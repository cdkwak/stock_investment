"""Offline transaction boundary for exact-date equity-fundamental Raw capture.

The historical dataset is immutable provider JSON in Landing.  This module
adds only a baseline-bound incremental overlay checkpoint; it never creates a
Normalized/Canonical projection and deliberately contains no HTTP transport.
An active runbook must supply an exact completed date, reviewed availability
and finality evidence, and a one-call capture function.
"""
...
NONNEGATIVE_FIELDS = frozenset({"TDD_CLSPRC", "PER", "PBR", "DPS", "DVD_YLD"})
MISSING_TOKEN = "-"
BASELINE_STATE = Path("data/state/pykrx_high_value_raw") / f"{DATASET}.json"
CHECKPOINT = Path("data/state/pykrx_high_value_raw") / f"{DATASET}_incremental.json"
JOURNAL = (
...
CHECKPOINT_SCHEMA = "pykrx_equity_fundamental_daily.incremental.v1"
OVERLAY_MODE = "IMMUTABLE_BASELINE_PLUS_EXACT_DATE_RAW"
JOURNAL_TERMINAL = {
    "SUCCEEDED",
    "SUCCEEDED_RECOVERED",
    "FAILED_FETCH",
...
class FundamentalGateError(RuntimeError):
...
def validate_authorization(
    ...
    *,
    target_date: str,
    source_policy: FundamentalSourcePolicy = SOURCE_POLICY,
) -> None:
    expected = _iso_date(target_date)
    if _iso_date(authorization.intended_date) != expected:
        raise FundamentalGateError("authorization intended date mismatch")
    if _iso_date(authorization.completed_date) != expected:
        raise FundamentalGateError("authorization completed date mismatch")
    if not authorization.provider_available:
        raise FundamentalGateError("provider availability is not confirmed")
    if (
    ...
    if authorization.finality_evidence != source_policy.finality_evidence_id:
        raise FundamentalGateError("typed source finality evidence mismatch")
    if (
        ...
        raise FundamentalGateError("call budget must be one call with retry zero")
...
def analyze_capture(capture: SourceCapture, *, target_date: str) -> dict[str, Any]:
    ...
    if capture.source != SOURCE:
        raise ValueError("fundamental source identity mismatch")
    if capture.source_operation != SOURCE_OPERATION:
        raise ValueError("fundamental source operation mismatch")
    if capture.source_scope != SOURCE_SCOPE:
        raise ValueError("fundamental source must cover ALL market scope")
    if _date_key(capture.source_date) != compact:
        raise ValueError("fundamental source date mismatch")
    if dict(capture.request_payload) != request_payload(compact):
        raise ValueError("fundamental exact-date request mismatch")
    if capture.http_status != 200:
        raise ValueError(f"fundamental HTTP status {capture.http_status}")
    return _analyze_body(capture.body, target_date=compact)


def _analyze_body(body: bytes, *, target_date: str) -> dict[str, Any]:
    if not isinstance(body, bytes) or not body:
        raise ValueError("fundamental capture body is empty")
    if body.lstrip().startswith(b"<"):
        raise ValueError("fundamental capture is HTML/restriction content")
    try:
        payload = json.loads(body)
    except Exception as error:
        raise ValueError("fundamental capture is not JSON") from error
    if not isinstance(payload, dict) or any(
        payload.get(name) for name in ("_error_code", "error", "errors")
    ):
        raise ValueError("fundamental provider error payload")
    rows = payload.get("output")
    if not isinstance(rows, list) or not rows:
        raise ValueError("fundamental valid-empty is not accepted")

    positions: dict[str, list[int]] = {}
    missing_value_counts = {field: 0 for field in NUMERIC_FIELDS}
    for ordinal, row in enumerate(rows, start=1):
        if not isinstance(row, dict) or not set(REQUIRED_FIELDS).issubset(row):
            raise ValueError("fundamental source schema mismatch")
        symbol = str(row["ISU_SRT_CD"]).strip()
        name = str(row["ISU_ABBRV"]).strip()
        if not symbol or row["ISU_SRT_CD"] is None:
            raise ValueError("fundamental empty security identity")
        if not name or row["ISU_ABBRV"] is None:
            raise ValueError("fundamental empty security name")
        positions.setdefault(symbol, []).append(ordinal)
        for field in NUMERIC_FIELDS:
            value = row[field]
            if value is None or not str(value).strip():
                raise ValueError(f"fundamental null field: {field}")
            parsed = _provider_number(value, field=field)
            if parsed is None:
                missing_value_counts[field] += 1
            elif field in NONNEGATIVE_FIELDS and parsed < 0:
                raise ValueError(f"fundamental out-of-range field: {field}")

    duplicate_groups: list[dict[str, Any]] = []
    for symbol, ordinals in positions.items():
        if len(ordinals) == 1:
            continue
        ...
        duplicate_groups.append(
            {
                ...
                "classification": (
                    "CONFLICTING_PROVIDER_DUPLICATE"
                    if differing
                    else "EXACT_PROVIDER_DUPLICATE"
                ),
    ...
    return {
        ...
        "rows": len(rows),
        "distinct_security_codes": len(positions),
        "body_sha256": hashlib.sha256(body).hexdigest(),
        "source_scope": SOURCE_SCOPE,
        "row_identity": "source_row_ordinal_1_based",
        "provider_duplicate_groups": duplicate_groups,
        "missing_value_counts": missing_value_counts,
        "completeness": "EXACT_ALL_MARKET_RESPONSE_NONEMPTY_SCHEMA_COMPLETE",
    }


def _provider_number(value: Any, *, field: str) -> Decimal | None:
    token = str(value).strip()
    if token == MISSING_TOKEN:
        if field == "TDD_CLSPRC":
            raise ValueError("fundamental missing close price")
        return None
    try:
        parsed = Decimal(token.replace(",", ""))
    except InvalidOperation as error:
        raise ValueError(f"fundamental non-numeric field: {field}") from error
    if not parsed.is_finite():
        raise ValueError(f"fundamental non-finite field: {field}")
    return parsed
...
def recover_incomplete(project_root: Path) -> str:
    ...
    if journal.get("dataset") != DATASET:
        raise FundamentalGateError("fundamental journal identity mismatch")
    ...
    if not response_value:
        ...
        _atomic_json(journal_path, journal)
        raise FundamentalGateError("incomplete call has no durable response")
    response_path = _safe_project_path(project_root, response_value)
    if not response_path.is_file() or response_path.is_symlink():
        raise FundamentalGateError("recovery response path is unavailable")
    ...
    if analysis["body_sha256"] != journal.get("response_sha256"):
        raise FundamentalGateError("recovery response hash mismatch")
...
def run_fundamental_incremental(
    project_root: Path,
    *,
    target_date: str,
    ...
    if action != "RUN_REQUIRED":
        raise FundamentalGateError(action)
    if authorization is None:
        raise FundamentalGateError("exact-date finality authorization is required")
    ...
    if capture_fn is None:
        raise FundamentalGateError("authorized capture function is required")

    transaction_id = f"fundamental_{compact}_{uuid4().hex}"
    ...
    try:
        ...
        if analysis["body_sha256"] != journal["response_sha256"]:
            raise ValueError("fundamental captured bytes changed")
        provenance_path = response_path.with_name("provenance.json")
        _atomic_json_new(
            provenance_path,
            _provenance(project_root, response_path, capture, analysis),
...
def _checkpoint_with_record(
    ...
    if checkpoint is None:
        ...
        raise FundamentalGateError("fundamental baseline state changed")
    completed = dict(checkpoint["completed"])
    if compact in completed:
        if dict(completed[compact]) != dict(record):
            raise FundamentalGateError("fundamental date already has another capture")
        return checkpoint
...
def _baseline_summary(project_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or not completed
        or payload.get("normalized_writes") is not False
    ):
        raise FundamentalGateError("fundamental baseline checkpoint is invalid")
    return {
...
def _load_checkpoint(project_root: Path) -> dict[str, Any] | None:
    ...
    if (
        ...
        or checkpoint.get("overlay_mode") != OVERLAY_MODE
        or not isinstance(checkpoint.get("completed"), dict)
    ):
        raise FundamentalGateError("fundamental incremental checkpoint mismatch")
    return checkpoint
...
def _verify_baseline_record(
    ...
    if (
        ...
        raise FundamentalGateError("fundamental baseline Landing mismatch")
    retained_groups = record.get("provider_duplicate_groups")
    if retained_groups is not None and retained_groups != analysis["provider_duplicate_groups"]:
        raise FundamentalGateError("fundamental baseline duplicate metadata mismatch")
...
def _verify_incremental_record(
    ...
    if provenance is None:
        raise FundamentalGateError("fundamental provenance is missing")
    ...
    if (
        ...
        or record.get("provider_duplicate_groups")
        != analysis["provider_duplicate_groups"]
    ):
        raise FundamentalGateError("fundamental incremental Landing mismatch")
...
def _record(
    ...
    return {
        "classification": (
            "SUCCESS_INCREMENTAL_RAW_WITH_PROVIDER_DUPLICATE"
            if analysis["provider_duplicate_groups"]
            else "SUCCESS_INCREMENTAL_RAW"
        ),
        "rows": analysis["rows"],
        "distinct_security_codes": analysis["distinct_security_codes"],
