"""UR-214's independent 19:00 KST USD/KRW mobile-home window."""
...
OPERATION_ID = "UR-214"
WINDOW_ID = "2026-08-21T19:00:00+09:00"
MANIFEST_PATH = Path("data/state/naver_mobile_home_ur214_activation.json")
STATE_PATH = Path("data/state/naver_mobile_home_ur214_window.json")
LANDING_ROOT = Path("data/landing/naver_mobile_home/ur214")
...
def ensure_manifest(root: Path) -> Path:
    ...
    if json.loads(path.read_text(encoding="utf-8")) != expected:
        raise RuntimeError("UR-214 activation manifest differs from approved scope")
    return path
...
def read_manifest(root: Path) -> dict[str, object]:
    try:
        ...
        raise RuntimeError("UR-214 activation manifest is unreadable") from error
