...
(function () {
  ...
  const indicatorLabels = {
    drawdown252: "252일 낙폭", disp60: "60일 이격", rsi14: "RSI14", volidx_pct: "변동성 백분위",
  };
  ...
  function renderLeaderboard(payload) {
    ...
    $("leaderboard-body").innerHTML = candidates.map((candidate) => {
      ...
      return `<tr data-candidate="${encodeURIComponent(candidate.id || "")}" tabindex="0" role="button" aria-label="${esc(candidate.name || candidate.id)} 상세 보기">
        ...
        <td>${pct(holdout.hit_60, 0)}</td><td>${pct(holdout.vol_60)}</td><td class="${valueClass(holdout.mdd_60)}">${pct(holdout.mdd_60)}</td>
  ...
  function renderLevelChart(levels) {
    ...
    const points = (levels || []).map((level, index) => ({
      t: `2000-01-${String(index + 1).padStart(2, "0")}`,
      v: (level.holdout || {}).mean_60,
      level: level.level,
    })).filter((point) => finite(point.v));
    window.SIChart.renderLineChart(host, [], {
      height: 132, xMode: "index", ariaLabel: "단계별 홀드아웃 60일 평균 막대 차트",
      emptyMessage: "홀드아웃 단계 관측이 2개 이상이면 표시됩니다.",
      series: [{ key: "level", label: "60일 평균", color: "#a8621a", type: "bar", points }],
      axisFormatter: (value) => `${(Number(value) * 100).toFixed(1)}%`,
  ...
  function currentMarkup(candidate) {
    ...
    const facts = Object.entries(indicators).map(([key, value]) => {
      const shown = key === "rsi14" ? number(value, 1) : pct(value);
    ...
    return `<div class="research-current-strip">
      ...
      <div class="current-main"><b>${esc(candidate.name || candidate.id)}</b><span>점수 <b class="num">${number(current.score, 1)}</b></span><span><b class="num">${number(current.level, 0)}/${number(current.max_level, 0)}</b>단계</span><span>노출 <b class="num">${pct(current.exposure, 0)}</b></span></div>
      <div class="current-facts">${facts || "지표 없음"}</div>
      <div class="current-analog">과거 같은 단계 n=${number(analog.n, 0)} → 60일 평균 <b class="num ${valueClass(analog.mean_60)}">${pct(analog.mean_60)}</b>, 상승확률 <b class="num">${pct(analog.hit_60, 0)}</b></div>
    </div>`;
  }
  ...
  function renderForward(payload) {
    ...
    $("forward-content").innerHTML = (payload.groups || []).map((group, groupIndex) => `<details class="forward-version" ${groupIndex === 0 ? "open" : ""}><summary>규칙 버전 ${esc(String(group.rules_version || "미상").slice(0, 8))}<span>최근 신호 ${esc(group.newest_as_of || "—")}</span></summary>${(group.candidates || []).map((candidate) => {
      ...
      const rows = (candidate.rows || []).map((row) => `<tr><td>${esc(row.as_of || "—")}</td><td>${number(row.level, 0)}</td><td>${pct(row.exposure, 0)}</td>${[20, 60, 90].map((horizon) => `<td class="${valueClass(row[`return_${horizon}`])}">${row[`status_${horizon}`] === "실현" ? pct(row[`return_${horizon}`]) : "대기"}</td>`).join("")}</tr>`).join("");
      return `<div class="forward-candidate"><div class="forward-candidate-head"><b>${esc(candidate.name || candidate.candidate_id)}</b><span class="muted">${esc(basketLabels[candidate.basket] || candidate.basket || "—")} · ${number((candidate.rows || []).length, 0)}행</span></div><div class="research-table-wrap"><table class="research-table forward-table"><thead><tr><th>기준일</th><th>단계</th><th>노출</th><th>실현 20일</th><th>실현 60일</th><th>실현 90일</th></tr></thead><tbody>${rows}</tbody></table></div><div class="forward-summary">${summaryLine}</div></div>`;
    }).join("")}</details>`).join("");
  }
