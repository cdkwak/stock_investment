...
| Korean Treasury | The 홈 `한국 3Y` and `한국 10Y` tiles (split 2026-09-05 from one combined tile) currently display percentages. `src/stock_web/api/home_data.py` selects BOK `bok_ecos_kr_treasury_yield_source_observation` when its latest curve is at least as recent as Toss `kr_treasury_yield_daily`, otherwise Toss. On 2026-09-05 the retained maxima were BOK 2026-08-13 and Toss 2026-09-03, so the live readback selected `Toss 국채 09-03`. Visible payload fields per tile are `value` (`…%`), `change_label` (bp change), and `source_label`/`sub_note` (provider and date). BOK 817Y002 permanent finality remains unknown, and a docs search found no BOK ECOS or Toss Treasury terms/numeric-display rights-gate record: **표시 중이나 권리 게이트 기록 없음 — 사용자 확인 필요**. Yahoo quote indices, futures prices, and older curves are not substitutes for official BOK yields. |
...
|---|---|---|
| Korean Treasury display lacks a recorded rights gate | The 홈 `한국 3Y`·`한국 10Y` percent values, bp changes, and provider/date labels are already visible, but the repository has no BOK ECOS or Toss Treasury terms/numeric-display rights decision | User confirmation is required before treating the current local display as authorized or expanding it; Data still owns finality/PIT and source semantics |
...
3. Keep the current Korean sovereign-yield tile source-labelled and local-only,
   but do not expand it until the missing BOK ECOS display-rights gate is
   confirmed by the user. Keep quote-index, futures-price, official-yield,
   curve-spread, and bond-ETF return semantics separate; finality/PIT limits
   remain in force.
