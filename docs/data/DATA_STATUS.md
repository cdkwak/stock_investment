...
| Runtime health | Managed Health is `39/39` at the 2026-09-02 KST readback. A new retry-zero Yahoo occurrence returned three HTTP 200 responses and atomically promoted SP500, NASDAQ Composite, and NASDAQ-100 from 2026-08-28 to the latest completed session 2026-08-31; its same-target replay was API 0 with the dataset digest unchanged. The independent FRED VIX phase then promoted one `VIXCLS` observation for 2026-08-31 with one HTTP 200 response; FRED yields and FX were pre-network no-ops, and the full FRED replay was API 0. All 39 automation-enabled rows are now `CURRENT` or contract-valid `EXPECTED_LAG`, with no managed `STALE` row. |
| Scheduler baseline | Thirteen enabled Data definitions remain; no extra Windows task was created. Release policy now matches the installed `pythonw.exe` actions and direct Toss UR246 action, and live readback reports definition mismatch `0`. The 2026-09-02 09:10 KR task and manually re-run idempotent global-futures task both ended with result `0`; the 14:10/20:30 KR slots retain prior nonzero results until their natural times. All 39 automation-enabled datasets map to 19 logical lanes. The BOK source observation stays on its correct 17:10 task and exits API-zero because its three-batch gate is reached. The separate read-only KB account task remains outside the 80-row market universe. Stable relations are in [Scheduler Data Map](SCHEDULER_DATA_MAP.md). |
...
| Retained-data access | Exactly one of 56 immediate Normalized dataset directories is inaccessible: `kr_kospi200_futures_investor_net_purchase_daily`. The 2026 Parquet exists but Windows denies directory/file reads, so its historical audit is quarantined. An exact ACL reset requires explicit user approval; after approval, hash and audit before allowing the dataset back into evidence. |
| Release readiness | The fresh 2026-09-02 09:50 KST offline read-only gate returned `FAIL` without external calls or Data/scheduler mutation. Data-root/schema/backtest/local-cache/freshness/required-result/GUI/user-byte checks passed, and scheduler definition mismatch stayed `0`. Exact blockers are `HEALTH_RECEIPT_RECONCILIATION`, `SCHEDULER_READ_ONLY_STATUS` with three retained nonzero results, and `DUE_OCCURRENCE_OUTCOMES` with 9/10 groups complete. The failed KB receipt owns the one due-group failure; the 14:10/20:30 KR slots must run naturally. The inaccessible Normalized dataset is a separate retained-audit gate not enumerated by this release script. |
...
1. Observe the 14:10 and 20:30 KR bundle occurrences. Confirm each exact receipt
   completes and managed Health remains `39/39`; do not run a future slot early.
...
3. After explicit user approval, reset only the inaccessible futures-investor
   dataset ACL, verify its content hash and retained-data audit, then rerun the
   historical test. Until then, quarantine only that dataset and dependent audit.
