...
| Retained-data access | Exactly one of 56 immediate Normalized dataset directories is inaccessible: `kr_kospi200_futures_investor_net_purchase_daily`. The 2026 Parquet exists but Windows denies directory/file reads, so its historical audit is quarantined. The user approved an exact owner/inheritance repair, but the current process has no administrator token and Windows rejected the repair. Apply it only when an elevated token is available, then hash and audit before allowing the dataset back into evidence. A separate inaccessible state-file ACL was repaired without elevation and with its SHA-256 unchanged. |
