...
| Registry | 42 operation entries, 85 universe rows, 41 automation-enabled datasets |
...
| Scheduler baseline | Thirteen enabled Data definitions remain; no extra Windows task was created. Release policy matches the installed `pythonw.exe` actions and direct Toss UR246 action, and live readback reports definition mismatch `0`. The 09:10 KR task and idempotent global-futures run ended with result `0`. The natural 14:10 bundle used five API calls and successfully advanced Canonical Equity and Lending through 2026-09-01 while Short Selling was already current; it terminalized result `1` only because the then-current Health projection incorrectly treated pre-20:30 downstream rows as stale. That classifier is fixed. `KR_ETF_PRICE_DAILY` is now the second lane in the existing 20:30 bundle, immediately after Canonical Equity; no new Windows task is required. All 41 automation-enabled datasets map to 20 logical lanes. The BOK source observation stays on its correct 17:10 task and exits API-zero because its three-batch gate is reached. The separate read-only KB account task remains outside the 85-row market universe. Stable relations are in [Scheduler Data Map](SCHEDULER_DATA_MAP.md). |
...
1. Observe the 20:30 KR bundle occurrence. Confirm its exact receipt completes
   and managed Health reflects all `41` automation-enabled datasets; do not run the slot early. Preserve the
...
| Korean issuer financial health | [Quarterly fundamentals source options](sources/FUNDAMENTALS_SOURCE_OPTIONS.md), [OpenDART source record](sources/opendart/README.md), and [target-price consensus research](sources/TARGET_PRICE_CONSENSUS.md). The fundamentals route remains `NO_COMPLIANT_NORMALIZED_SOURCE / NO_LIVE_COMMAND`; OpenDART key name and Raw pilot exist, but rights/revision/availability gates prohibit scanner promotion. Target-price consensus remains separate, manual, non-predictive reference data. |
