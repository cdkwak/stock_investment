...
| BOK finality and Korean Treasury display gate | `THREE_BATCH_GATE_REVIEWED / BOUNDED_THREE_BATCH_AVAILABILITY_CONFIRMED / PERMANENT_FINALITY_UNKNOWN`. The 2026-08-26..28 BOK batches each selected the same-day provider-native date across all six tenors with six data calls, one separate UI call, retry zero, and no Normalized write. Both next-provider-day comparisons were field/byte `SAME`; all UI table markers were `N/N`. The 2026-08-29 scheduler receipt stopped at 3 to 3 with API calls zero. Automatic expected latest, permanent finality, and predictive/PIT use remain unsupported. Contrary to the earlier numeric-display prohibition, `src/stock_web/api/home_data.py` renders the 홈 `한국 3Y · 10Y` tile from BOK `bok_ecos_kr_treasury_yield_source_observation` when it is at least as recent as Toss `kr_treasury_yield_daily`, otherwise Toss, including percent values, bp changes, and source/date labels. On 2026-09-05 the retained maxima were BOK 2026-08-13 and Toss 2026-09-03, so the live readback selected `Toss 국채 09-03`. A docs search found no BOK ECOS or Toss Treasury terms/numeric-display rights-gate record: **표시 중이나 권리 게이트 기록 없음 — 사용자 확인 필요**. Do not expand the display or treat it as authorization for redistribution. |
...
| Yahoo daily registry | SOXX and EWY are retained. SOXL, TQQQ, QLD, TLT, QQQ, and SPY are `REGISTERED_NOT_YET_COLLECTED` under the existing 06:10 ETF task; leverage multiples are contract data (`3,3,2,1,1,1`) rather than name inference. SOX (`^SOX`), DOW_JONES (`^DJI`), SP500_FUTURES (`ES=F`), DOW_FUTURES (`YM=F`), and DOLLAR_INDEX_FUTURES (`DX=F`) remain separately registered. Each Yahoo symbol uses an independent Landing-first, identity-validated CAS promotion; a failed symbol preserves prior rows and does not block valid peers. |
| Korean watchlist ETFs | `kr_etf_master` and `kr_etf_price_daily` are `AUTOMATION_ACTIVE / KR_ETF_PRICE_DAILY / DISPLAY_ONLY / PIT_BLOCKED`. The verified first run retained `123320` and `243880` for 2026-08-24..2026-09-02 with five pykrx calls, Landing-first read-back, atomic Parquet, checkpoint, and state. The 20:30 lane resolves the union of local KRX/ETF watchlist symbols and retained master symbols, caps it at 10, skips already-current symbols at API 0, and limits each catch-up to 30 XKRX sessions. A missing target-session row is `EXPECTED_PROVIDER_LAG`, not failure. |
| Same-day Korean equity closes | `kr_equity_price_provisional_daily` is `IMPLEMENTED_OFFLINE / KR_EQUITY_PROVISIONAL_DAILY / DISPLAY_AND_CONDITION_ALERTS_ONLY / NON_PREDICTIVE`. The 20:30 lane makes exactly two market-wide pykrx calls, captures both frames Landing-first, and atomically writes same-session OHLCV. Canonical D+1 rows always take reader precedence; retained target replay is API 0 and two valid-empty frames are `EXPECTED_PROVIDER_LAG`. The first bounded human run remains pending. |
| Equity current valuation observation | One retry-zero KRX `MDCSTAT03501` ALL-market observation for 2026-08-25 retained 2,719 unique source rows with zero duplicates. It is exact-date descriptive PER/PBR evidence only (`PIT_LIMITED_FIRST_OBSERVED_ONLY`), not canonical history, Forward EPS or a relative-value judgment. The 09:10 child introduced in v4 is retained unchanged in active bundle v7 and catches up one completed session at a time; valid-empty is expected provider lag and retries only at the next natural occurrence, while auth/network/schema failures remain typed lane failures. |
| Yahoo direct-30m switch gate | `PARTIAL_KEEP_NATIVE_15M` on 2026-08-27. Four retry-zero calls confirmed exact symbol/Cboe exchange/`America/Chicago`/`INDEX`/`dataGranularity=30m` identity. `^VIX` passed the `:00/:30` completed-grid, finite OHLC, retained-15m same-end close, and `:02/:32` selector checks. `^FVX/^TNX/^TYX` instead exposed provider-native start-minute offsets `:20/:50` plus an as-retrieved `:45` quote row, yielding zero `:00/:30` completed candidates and no same-end retained-15m comparison. No mixed interval or code/GUI/scheduler change is allowed from this result; all four routes remain native 15m and retained history remains `STATIC_COMPLETE / NO_REFRESH`. Evidence: 제거됨 (backup/repo-cleanup-phase2-20260903 브랜치에 보존); four immutable responses remain under `data/landing/yahoo_market_30m_validation/20260827/`. |
| Accounts | Toss read-only refresh is installed for 07:00 and retains its strict successful receipt/digest contract. KB account data remains outside the 87-row market universe. The natural 2026-09-02 07:10 KB occurrence failed closed after one supplier call with `KB_ACCOUNT_SUPPLIER_FAILED`, preserved the prior valid snapshot, and retained the immutable failed occurrence receipt. A separate supported manual read-only refresh later succeeded with one supplier call and updated the identifier-free local snapshot; it does not rewrite the failed scheduler receipt or claim scheduled success. Continue one-call idempotency, identifier removal, exact receipt/digest binding, and prior-valid preservation. The seven `kb_*_snapshot` universe rows are separate market snapshots. |
...
| Canonical equity and events | [Canonical daily contract](operations/CANONICAL_EQUITY_DAILY.md), [canonical incremental](operations/CANONICAL_EQUITY_DAILY_INCREMENTAL.md), [same-day provisional closes](operations/KR_EQUITY_PROVISIONAL_DAILY.md), [dividend append](operations/DIVIDEND_OBSERVATION_APPEND.md) |
| Global/current display | [Global current refresh](operations/GLOBAL_CURRENT_REFRESH.md), [global 30-minute route](operations/GLOBAL_MARKET_CURRENT_60M.md), [global ETF onboarding](operations/GLOBAL_ETF_SOXX_ONBOARDING.md), [2026-09-02 new-symbol bounded collection](operations/GLOBAL_NEW_SYMBOLS_20260902.md), [Korean watchlist ETF daily](operations/KR_ETF_DAILY.md), [FDR future display](operations/FDR_FUTURE_DISPLAY_DAILY.md), [Yahoo historical/native-lane evidence](operations/YAHOO_MARKET_15M.md) |
...
| Korean issuer financial health | [Quarterly fundamentals source](sources/FUNDAMENTALS_SOURCE_OPTIONS.md), [OpenDART source record](sources/opendart/README.md), [weekly lane](operations/KR_FUNDAMENTALS_WEEKLY.md), and [target-price consensus research](sources/TARGET_PRICE_CONSENSUS.md). OpenDART `kr_fundamentals_quarterly` is retained in Normalized with 1,506 rows for 158 securities as read-only verified on 2026-09-05. `refresh_kr_fundamentals.py` is the bounded Landing-first/two-step collector, and `KR_FUNDAMENTALS_WEEKLY` runs in the 20:30 bundle on the last XKRX session of each ISO week. The 종목 financial table and scanner consume the dataset, including `debt_ratio_pct`. The 2026-09-03 source record links the official terms and records user acceptance of personal-use retention; redistribution is not authorized. Filing availability/revision/PIT remain unresolved, so use stays `DISPLAY_AND_SCANNER_ONLY / PIT_BLOCKED`. Target-price consensus remains separate, manual, non-predictive reference data. |
| Market sessions and U.S. option sentiment | [Market-session rule evidence](sources/MARKET_SESSION_RULES.md) is calendar evidence, not a provider-bar mapping. [U.S. option P/C source decision](sources/us_option_pcr/US_OPTION_PCR_SOURCE_DECISION.md) remains `UNSUPPORTED / NUMERIC_USE_FORBIDDEN`; do not substitute Yahoo or Korean P/C data. |
