...
1. Observe the next natural global and 20:30 KR bundle occurrences. Confirm the
   recovered global-index and FRED VIX lanes remain API-zero while current, the
   13-lane KR receipt completes, and managed Health remains `39/39`.
...
3. Observe the next date-keyed KB `SSQM2952` account occurrence at 07:10 and
   reverify its identifier-free receipt, exact snapshot digest, one-call budget,
