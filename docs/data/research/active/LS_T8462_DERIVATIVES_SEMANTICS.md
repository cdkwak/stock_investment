...
| `semantic_closure` | `CLOSED_WITH_LIMITS` |
| `availability_status` | `HISTORICAL_RESEARCH_ONLY_NON_PREDICTIVE` |
| `normalized_status` | `FORBIDDEN_PENDING_FINALITY_REVISION_AND_PIT_OPERATION` |

The pilot and historical semantic audit are closed and must not be rerun by
default. Ongoing work routes to the separate
[daily Raw runbook](../../operations/LS_T8462_DAILY_RAW_COLLECTION.md).

## Decision

LS `t8462` returns real dated investor rows for KOSPI200 and mini-KOSPI200
futures, calls, and puts. Authenticated official-KRX comparison establishes
`sv_*` as one-contract net purchase across products and dates. It supports
`sa_*` as signed 100-million-KRW net purchase and `U` as the KRX `ALL` scope
through multi-product, multi-date empirical matches. Three-date KRX session
comparisons confirm `D=REGULAR` and `N=NIGHT` for KOSPI200 futures, KOSPI200
CALL, and mini-KOSPI200 futures, with a non-equivalent institution/other
allocation boundary in some futures `D/N` scopes. These are not official LS definitions:
session cut-offs/finalization, category equivalence, publication/revision timing,
and PIT policy remain unresolved, so safe normalization is still forbidden.

Analysis must preserve `sv_18` institution and `sv_07` other corporation as
separate `LS_NATIVE_CATEGORY` source values. The analysis-only
`institutional_complex = sv_18 + sv_07` feature is the default for validated
KOSPI200-futures `D/N` cross-provider and backtest use; it is not a Raw field or
a provider category. See the
[analysis feature policy](../../config/LS_T8462_ANALYSIS_FEATURES.md).

| Product/scope | Verified result | Classification |
|---|---|---|
| KOSPI200 futures, `D` | 2025-07-18..2026-08-14 Raw; three dates match KRX regular | `LIMITED_HISTORY` + `SESSION_CONFIRMED_EMPIRICAL_REGULAR_WITH_CATEGORY_BOUNDARY` |
| KOSPI200 futures, `N` | 2025-07-18..2026-08-14 Raw; three dates match KRX night | `LIMITED_HISTORY` + `SESSION_CONFIRMED_EMPIRICAL_NIGHT_WITH_CATEGORY_BOUNDARY` |
| KOSPI200 CALL, `D/N/U` | Three dates match KRX regular/night/all | `SESSION_CONFIRMED_EMPIRICAL_MULTI_DATE` |
| Mini-KOSPI200 futures, `D/N/U` | Three dates match KRX regular/night/all; institution/other boundary in some `D/N` | `SESSION_CONFIRMED_EMPIRICAL_MULTI_DATE_WITH_CATEGORY_BOUNDARY` |
| KOSPI200 PUT, `D/N/U` | Three dates; 45/45 category/combined contract comparisons exact | `SESSION_CONFIRMED_EMPIRICAL_MULTI_DATE` |
| Mini-KOSPI200 CALL, `D/N/U` | Three dates; 45/45 category/combined contract comparisons exact | `SESSION_CONFIRMED_EMPIRICAL_MULTI_DATE` |
| Mini-KOSPI200 PUT, `D/N/U` | Three dates; 45/45 category/combined contract comparisons exact | `SESSION_CONFIRMED_EMPIRICAL_MULTI_DATE` |
...
### KRX unit, sign, and `U`-scope cross-check

The authenticated official KRX `[15007]` screen was fixed to market `ALL`,
volume unit contracts, and amount unit million KRW. Comparisons used KOSPI200
futures on 2026-01-02, 2026-07-31, and 2026-08-13 plus KOSPI200 CALL,
KOSPI200 PUT, and mini-KOSPI200 futures on 2026-08-13. Individual, foreign,
institution total, and other corporation produced 24 direct points.

All 24 `sv_*` points exactly matched the KRX contract-unit net-purchase values,
so the quantity unit is `CONFIRMED_EMPIRICAL_MULTI_PRODUCT_MULTI_DATE_CONTRACT`.
For the same points, `LS sa_* × 100` matched KRX million-KRW net purchase within
source rounding; residuals were -47..+45 million KRW. The amount classification
was initially inferred at 100 million KRW. The subsequent three-date,
all-session CALL and mini-futures comparison promotes signed net purchase to
`CONFIRMED_EMPIRICAL_MULTI_PRODUCT_MULTI_DATE_100_MILLION_KRW_NET_PURCHASE`.
This remains empirical and is not an official LS unit/sign statement.

The same evidence confirms `U=KRX ALL` across products and dates. Separate
three-date KOSPI200-futures checks confirm `D=KRX REGULAR` and `N=KRX NIGHT`:
individual/foreign contract net purchase matched 12/12, and institution plus
other corporation matched 6/6 after combination. KRX and LS allocated 3, 34,
and 53 contracts differently between institution total and other corporation
on the three dates, so those two labels are not declared source-equivalent.
Session cut-offs and finalization remain undocumented, and no universal
`U=D+N` repair or derivation rule is authorized.
...
[option-U institution differences](../../../archive/data/evidence/2026-08-data-phase/ls/LS_T8462_OPTION_U_INSTITUTION_DIFFERENCES.csv).
...
Direct KRX checks on three mismatch dates classify the observed option-`U`
pattern more specifically as `AGGREGATE_FIELD_SEMANTICS`: KRX institution
equals the retained LS institution-component sum rather than `sv_18`, while
KRX other corporation equals LS `sv_07`. Two post-boundary normal dates match
both LS forms. This does not authorize selection or repair of either LS field.

The 2026-08-13 KRX option `ALL` volume net purchase equals `CALL + PUT` exactly
for the four compared investor groups. Amount is exact for three groups and
differs by one displayed million-KRW unit for individual. The retained LS CALL
and PUT `U` rows reproduce the components: their quantities sum exactly to KRX
`ALL`, and their amounts at the inferred `×100` scale remain within rounding.
Detailed values and immutable LS hashes are retained in the
[multi-product validation evidence](../../../archive/data/evidence/2026-08-data-phase/ls/LS_T8462_KRX_MULTI_PRODUCT_VALIDATION_20260817.md).
