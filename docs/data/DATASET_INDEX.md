...
| `kr_corp_code_map`, `kr_fundamentals_quarterly` | OpenDART official API `corpCode.xml`, `fnlttSinglAcntAll.json`; key loaded from the environment without disclosure | `kr_fundamentals_quarterly`: 1,506 rows / 158 securities, read-only verified 2026-09-05 | `kr_fundamentals.py` v1; identity map `(corp_code)` and revision-preserving quarterly key `(symbol,bsns_year,reprt_code,fs_div,rcept_no)`; collector `scripts/manual/collect/refresh_kr_fundamentals.py` | immutable `data/landing/opendart/kr_fundamentals_quarterly/`; candidate staging; `data/normalized/{kr_corp_code_map,kr_fundamentals_quarterly}/` | `WEEKLY_AUTOMATION_ACTIVE / KR_FUNDAMENTALS_WEEKLY / DISPLAY_AND_SCANNER_ONLY / PIT_BLOCKED`; 20:30 bundle on the last XKRX session of the ISO week; 종목 financial table plus scanner `debt_ratio_pct`/four-quarter signs/revenue trend; CFS-first with OFS only after CFS `013`, Q4 de-cumulation, exact local call ledger; personal-use retention accepted 2026-09-03, redistribution not authorized; [source contract](sources/opendart/README.md), [collector/cadence](operations/KR_FUNDAMENTALS_WEEKLY.md) |
| `global_index_price_daily` | Per-symbol registry: Yahoo chart for SP500/NASDAQ_COMPOSITE/NASDAQ100/SOX/DOW_JONES/DOLLAR_INDEX/NIKKEI225/TAIEX/EURO_STOXX50/CAC40/HANG_SENG/DAX; Cboe public daily-history CSV for VIX9D/VIX3M/VIX6M/SKEW | Existing Yahoo broad indices retained; NIKKEI225/TAIEX/EURO_STOXX50/CAC40/HANG_SENG/DAX registered with exact currency, Yahoo exchange, exchange timezone, exchange-calendar identity and `ohlc_fill_from_close` (Yahoo's pre-2000 rows carry close only); full histories promoted: NIKKEI225 1985-01-02~, TAIEX 1997-08-01~, EURO_STOXX50 2007-04-02~, CAC 40 (France) `^FCHI` `PRICE` 1990-03-01~ (2026-09-06), HANG_SENG 1986-12-31~, DAX 1987-12-30~; Cboe term-index full-history onboarding/promote path active from 2026-09-04 | `global_market.py`; [Cboe index history](sources/cboe/INDEX_HISTORY.md) | Yahoo: `data/landing/global_current_refresh/`; CAC40 backfill: `data/landing/yahoo/chart/`; Cboe: `data/landing/cboe_index_history/<run_id>/<symbol>.csv`; `data/normalized/global_index_price_daily/` | mixed-provider `06:20 GLOBAL_INDEX_DAILY`; one retry-zero request per due symbol; Yahoo strict endpoints use the registered exchange calendar; Cboe files are full history and use source ticker equal to the Cboe symbol, nullable volume, hash-bound Landing, and isolated CAS promotion |
...
| `bok_ecos_kr_market_rate_daily` | BOK ECOS `817Y002/D`; `010300000` 회사채(3년, AA-), `010101000` 콜금리(1일, 전체거래) | 1995-01-03..2026-09-04; 8,042 corporate-bond rows and 8,041 call-rate rows | `bok_ecos_market_rates.py` v1; `(date,series)` key, annual-percent values, two separate series | immutable `data/landing/bok_ecos/kr_market_rates_daily/`; `data/normalized/bok_ecos_kr_market_rate_daily/year=YYYY/`; resumable backfill state | `MANUAL_ONLY / PRESERVED / PIT_BLOCKED`; no lane or scheduler. Requested `721Y001` was rejected for daily use because its item list exposes only A/M/Q and `721Y001/D` returned `INFO-200`; see [runbook](operations/BOK_ECOS_MARKET_RATES.md). |
...
| `kr_etf_investor_flow_daily` | KRX MDCSTAT04902[13207] via pykrx ETF investor trading-value endpoint | no retained rows before the coordinator's first live run | `kr_etf_investor_flow.py` v1; `(date,symbol)`; signed `기관/기타법인/개인/외국인/전체` net-purchase KRW amounts | `data/landing/pykrx/kr_etf_investor_flow_daily/`; `data/normalized/kr_etf_investor_flow_daily/symbol=<code>/year=<yyyy>/`; `artifacts/scheduler_logs/STOCK_DATA_KR_ETF_INVESTOR_FLOW_DAILY_last.json` | `MANUAL_READY / AUTOMATION_DISABLED / KR_ETF_INVESTOR_FLOW_DAILY / DESCRIPTIVE_ONLY / PIT_BLOCKED`; reuses the watchlist-plus-manual-account ETF universe, max 10 symbols, one call per symbol per at-most-10-calendar-day window, Landing-first and retained-key idempotent; [source note](sources/PYKRX_KR_ETF_INVESTOR_FLOW.md) |
