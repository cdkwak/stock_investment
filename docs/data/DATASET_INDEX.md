...
| `global_index_price_daily` | Per-symbol registry: Yahoo chart for SP500/NASDAQ_COMPOSITE/NASDAQ100/SOX/DOW_JONES/DOLLAR_INDEX; Cboe public daily-history CSV for VIX9D/VIX3M/VIX6M/SKEW | Yahoo broad indices retained; Cboe term-index full-history onboarding/promote path active from 2026-09-04 | `global_market.py`; [Cboe index history](sources/cboe/INDEX_HISTORY.md) | Yahoo: `data/landing/global_current_refresh/`; Cboe: `data/landing/cboe_index_history/<run_id>/<symbol>.csv`; `data/normalized/global_index_price_daily/` | mixed-provider `06:20 GLOBAL_INDEX_DAILY`; one retry-zero request per due symbol; Cboe files are full history and use source ticker equal to the Cboe symbol, nullable volume, hash-bound Landing, and isolated CAS promotion |
...
| `global_equity_price_daily` | Yahoo chart | SKHY registered; retained coverage begins only after the first bounded backfill/promotion | `global_equity.py` v1; SKHY is `EQUITY` / `DEPOSITARY_RECEIPT`, NASDAQ, ISIN `US78392B2060`, underlying `000660`; ADR ratio is unknown and remains `None` | `data/landing/global_current_refresh/`; `data/normalized/global_equity_price_daily/symbol=SKHY/year=YYYY/` | `GLOBAL_EQUITY_DAILY` / `GLOBAL_DAILY_FINAL`; one-call under-one-year onboarding via `yahoo_equity`; Yahoo identity requires USD and exchange `NMS/NGM/NASDAQ/NasdaqGM`; predictive use remains blocked |
| `tossinvest_us_quote_30m` | Toss Securities Open API `/api/v1/prices` | no retained rows before the first eligible scheduler run | inline v1 contract in `tossinvest_us_quotes.py`; `(retrieved_at,symbol)` run key; positive finite USD quote | `data/landing/tossinvest/us_quotes_30m/`; `data/normalized/tossinvest_us_quote_30m/date=YYYY-MM-DD/`; `artifacts/intraday/tossinvest_us_quotes_latest.json` | `TOSSINVEST_US_QUOTES_30M`; `GLOBAL_30M` (`PT30M`) during `[17:00,06:00)` KST; one call, retry zero, valid-empty/prior-valid preservation, descriptive pre/regular/after/closed hint; never an official bar or close |
...
The two new U.S. datasets above are intentionally not counted in the current
typed Health universe totals yet. That registry is owned by
`src/stock_data/contracts/registry.py`,
`src/stock_data/orchestration/daily_operations.py`, and
`src/stock_data/orchestration/dataset_universe.py`, all outside this task's hard
write scope. The coordinator must register both datasets and their lane/due-time
metadata there, then regenerate Health. Register `global_equity_price_daily` as
`정시` at the `GLOBAL_EQUITY_DAILY` / `GLOBAL_DAILY_FINAL` due slot, and
`tossinvest_us_quote_30m` as `정시` at each `TOSSINVEST_US_QUOTES_30M` /
`GLOBAL_30M` due boundary within `[17:00,06:00)` KST. Do not patch only the GUI
adapter or change `tests/unit/gui/test_gui_health.py` counts before that
authoritative registration is made.
