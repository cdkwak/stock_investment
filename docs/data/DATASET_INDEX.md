...
| Global index, FRED yields/FX, Treasury spread | Yahoo / FRED | index/FX through 2026-08-14; yields/spread through 2026-08-17 | `global_market.py` | `data/landing/global_current_refresh/`; normalized/derived artifacts | H.15/H.10 provider-publication policies; `FRED AUTOMATION_ACTIVE / predictive PIT blocked` |
| `global_etf_price_daily` / SOXX slice | Yahoo chart | 2025-08-18..2026-08-18; 252 rows / 252 XNYS sessions | `global_etf.py` | `data/normalized/global_etf_price_daily/`; capture-first Landing/state retained | `AUTOMATION_ACTIVE / SOXX_ONLY / PIT_BLOCKED`; pre-network API-0 replay; never substitute SOX |
...
| `global_commodity_futures_daily` | Yahoo chart API | dashboard subset NQ=F/GC=F/CL=F, 2025-08-18..2026-08-18; 252 rows each | `global_market.py` active; legacy name retained | `data/normalized/global_commodity_futures_daily/` plus immutable Landing | `AUTOMATION_ACTIVE / DESCRIPTIVE_EXPECTED_LAG / PIT_BLOCKED`; daily 22:10 KST, individual expiry and official settlement use prohibited; volume not used as a trusted activity signal |
