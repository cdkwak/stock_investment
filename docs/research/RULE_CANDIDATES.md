...
Each fit/hold-out result reports the row count `n` and an `independent_events` count: signal dates are pooled across the basket's series (KOSPI and KOSPI200 signalling on the same day count once), sorted, and split into episodes wherever consecutive signal dates are more than 90 calendar days apart (about 60 sessions, the 60-day return horizon, so episodes have non-overlapping forward windows). The nine predeclared cycle buckets (1997–98, 2000–02, 2008–09, 2011, 2015–16, 2018, 2020, 2022, 2025–26) are reported separately as `cycles_with_signal` (buckets containing at least one signal) and `signals_outside_cycles` (signal rows falling in none of them) so the bucket count is never mistaken for an event count. The UI renders this as `n=60 (독립 사건 5)` and appends simultaneous/unique counts when an upstream compound or foreign result already supplies both.
...
The schema-v2 leaderboard atomically writes `artifacts/research/rule_leaderboard/latest.json` and a `YYYYMMDD.json` copy. For a plain two-condition drawdown ladder, it reads (without recomputing) the matching compound-ladder grid row for every underlying of the basket (KR = KOSPI and KOSPI200, US_TECH = NASDAQ100, SEMIS = SOX) at the same thresholds/levels with the fixed combination 2x, exit a, costs on, base exposure 1.0 (`combination_label` carries that text). It records each underlying's hold-out final-wealth/baseline ratio and the summary's threshold×levels plateau verdict under `underlyings[]` (the scalar fields mirror the first underlying); semantically different or absent rows remain `unavailable`. Daily observations accumulate at `data/local/research/forward_test/signals.jsonl`. The `RESEARCH_FORWARD_TEST_DAILY` lane is the final member of the 20:30 KR market bundle, makes zero API calls, and fails independently of preceding lanes.
