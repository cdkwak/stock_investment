...
Status: `T8428_SOURCE_ERROR_BOUNDARY_STOP / T1633_RAW_BACKFILL_COMPLETE_WITH_LIMITS / NO_NORMALIZED_WRITES / NO_SCHEDULE`

As of: 2026-08-15 KST

This is the source-selection record for official LS OpenAPI data beyond the
separate [t8462 derivatives pilot](../../../../../data/research/active/LS_T8462_DERIVATIVES_SEMANTICS.md). It does
not authorize a historical backfill, WebSocket subscription, account query,
or order API. Current project state remains in
[DATA_STATUS](../../../../../data/DATA_STATUS.md).
...
The current official LS guide was rechecked directly on 2026-08-15. It labels
`custmoney`, `yecha`, `outmoney`, `trjango`, `futymoney`, `stkmoney`,
`mstkmoney`, `mbndmoney`, `bndmoney`, and `mmfmoney` in 억원, so their source
unit is officially `KRW 100 million`. It labels `date` only as 일자 and does
not publish an availability timestamp or revision policy. `bndsmoney` is named
`필러(구.단기채권)` without a usable current economic definition; `volume` has
no stated unit and `vol` is merely labelled 회전율. Official guide:
<https://openapi.ls-sec.co.kr/apiservice> (search `t8428`).

Research boundary:

- The ten explicitly labelled monetary balance/change fields may be retained as
  exact source observations and used for descriptive/cross-source analysis.
- They remain blocked from default predictive backtests because source-date
  publication timing and revisions are unknown. A future backtest may use them
  only after adopting an explicit conservative availability policy.
- `jisu`, `change`, `diff`, and `volume` overlap superior market datasets and
  are provenance fields rather than the primary research value of t8428.
- `bndsmoney`, `volume`, and `vol` must not be assigned stronger economic/unit
  semantics than the official labels. Source zeros remain zeros; they are not
  rewritten as missing values.

The separately authorized continuation run
`data/landing/ls/t8428_surrounding_funds_raw/plan=ee6629db1a35f965585e2d9abd31fd0d22fb9cbc52783341f318a7d41524948b/run=20260815T044846Z_ceca024d42384030a7a13d3a2075aac6`
adopted and hash-verified the five pages below, then made six t8428 calls with
one OAuth and retry 0. Five additional 500-row pages extended retained history
through 2006-06-01. The next continuation at cursor 2006-06-01 returned HTTP 200
with LS source code `IGW40014` and no rows; it was retained and not retried.
Across ten valid pages there are 5,000 physical rows, 4,991 unique dates,
nine identical cursor overlaps, and zero conflicting overlaps. Current verdict:
`SOURCE_ERROR_BOUNDARY_STOP / OBSERVED_EARLIEST_ONLY`; the reachable source floor
remains unproven, and no contract, Normalized write, or schedule is authorized.
...
The current official LS t1633 request/response tables were rechecked directly
on 2026-08-15. They explicitly define `gubun1=0` as 금액 and `gubun1=1` as
수량 and identify total/arbitrage/non-arbitrage buy, sell, and net fields. They
provide no multiplier or unit for `tot*`, `cha*`, `bcha*`, or `volume`.
Consequently amount remains `UNIT_INFERRED_CROSS_SOURCE_KRW_MILLION` and
quantity remains `UNIT_INFERRED_MAGNITUDE_THOUSAND_SHARES`; neither is promoted
to confirmed. No t1633 contract or Normalized promotion is authorized without
new official unit evidence. Official guide:
<https://openapi.ls-sec.co.kr/apiservice> (search `t1633`).

The bounded Raw acquisition is complete across two checkpoint-bound runs under
`data/landing/ls/t1633_program_trading_raw/`. It used OAuth twice across two
processes and made 54 business calls with retry 0. The second run adopted the
first run's hash-verified KOSPI amount stream rather than recollecting it.
All 54 Raw bodies have matching provenance hashes; retained Raw size is
4,446,952 bytes and the secret scans pass.

- KOSPI amount and quantity each retain 6,174 unique dates from 2001-08-01
  through 2026-08-14.
- KOSDAQ amount and quantity each retain 5,280 unique dates from 2003-01-13
  through 2026-08-14.
- Total unique market×measure daily observations are 22,908. Cursor boundary
  overlaps are value-identical and source arithmetic residuals remain within
  one source unit.
- LS signals continuation even after its historical floor and repeats the same
  one-row cursor. A fixed-point gate now treats that behavior as the stream
  terminal. Two extra KOSPI-amount fixed-point probes from the initial capped
  run are retained as redundant evidence and excluded from accepted pages.
- The resulting floor verdict is `FIXED_POINT_SOURCE_FLOOR`: 2001-08-01 for
  KOSPI and 2003-01-13 for KOSDAQ within the requested 2000-01-01 lower bound.
- Official schema confirms amount versus quantity but still omits multipliers.
  Amount remains `UNIT_INFERRED_CROSS_SOURCE_KRW_MILLION`; quantity remains
  `UNIT_INFERRED_MAGNITUDE_THOUSAND_SHARES`. These labels are not official unit
  confirmation, so no contract or Normalized promotion is authorized.
...
| Program t1633 | same endpoint; KOSPI/KOSDAQ, amount/quantity, value/cumulative, daily/weekly/monthly, `fdate/tdate`, continuation date | date/index; total, arbitrage and non-arbitrage buy/sell/net; volume | Raw complete: KOSPI 6,174 dates/measure from 2001-08-01; KOSDAQ 5,280/measure from 2003-01-13; fixed-point floors | Field mapping confirmed; amount/quantity multipliers remain cross-source/magnitude inferred, not officially stated | 22,908 unique market×measure observations retained | `RAW_BACKFILL_COMPLETE_WITH_LIMITS`; contract/Normalized gated on official multiplier evidence |
...
| Surrounding funds t8428 | `/stock/investinfo`; `fdate/tdate`, series selector, market, count and `key_date` continuation | date/index/turnover; customer deposits, change, receivables, credit balance, futures deposits, stock/mixed/bond/MMF funds | Ten verified 500-row pages reach 2006-06-01; nine identical cursor overlaps; next continuation failed with source `IGW40014` | Official monetary units are KRW 100 million; turnover percent. Calendar differs slightly from equity sessions | Complements lagged KB liquidity snapshot; 4,991 unique Raw dates retained | `SOURCE_ERROR_BOUNDARY_STOP / OBSERVED_EARLIEST_ONLY`; no retry without new LS evidence |
