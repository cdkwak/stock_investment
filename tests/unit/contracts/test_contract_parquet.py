...
def two_year_frame(close=105.0):
    return pd.DataFrame(
        [
            ["2025-12-31", "SP500", "^GSPC", 100., 110., 90., close, 1000],
            ["2026-01-02", "SP500", "^GSPC", 100., 110., 90., close, 1000],
        ],
        columns=GLOBAL_INDEX_PRICE_DAILY.column_names,
    )
...
def test_contract_read_preserves_clean_generation_inventory_and_timestamps(
    tmp_path: Path,
) -> None:
    contract = physical_contract()
    value = pd.DataFrame(
    ...
    root = tmp_path / "immutable-generation"
    contract_parquet.write_dataset_atomic(
        value, root, contract, validate_physical,
    )

    def inventory() -> tuple[tuple[str, bool, int, int, bytes | None], ...]:
    ...
    assert not (root / ".partition-generation.lock").exists()
...
def test_contract_storage_recovers_whole_generation_after_interruption(
    monkeypatch, tmp_path: Path,
) -> None:
    root = tmp_path / "global"
    previous = two_year_frame()
    candidate = two_year_frame(106.)
    contract_parquet.write_dataset_atomic(
        previous, root, GLOBAL_INDEX_PRICE_DAILY, validate_global_index,
    )
    ...
    with pytest.raises(KeyboardInterrupt, match="simulated"):
        contract_parquet.write_dataset_atomic(
            candidate, root, GLOBAL_INDEX_PRICE_DAILY, validate_global_index,
        )
    assert (root / ".partition-generation.json").is_file()

    monkeypatch.undo()
    pd.testing.assert_frame_equal(
        contract_parquet.read_dataset(
            root, GLOBAL_INDEX_PRICE_DAILY, validate_global_index,
        ),
        previous,
    )
