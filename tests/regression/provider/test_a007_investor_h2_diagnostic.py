...
from scripts.manual.diagnostic import a007_investor_h2_diagnostic_support as support
from scripts.manual.diagnostic import diagnose_a007_investor_h2 as runner
from scripts.manual.pilot.pykrx_short_selling_pilot_support import PilotStopped


FIXTURE_DATES = ("20120105", "20120106", "20140103")
...
def test_h2_plan_is_frozen_to_one_kospi_volume_request():
    assert support.SCOPE == {
        "strtDd": "20120105", "endDd": "20140103", "inqCondTpCd": 1, "mktTpCd": 1,
    }
    assert support.EXPECTED_DATE_COUNT == 494
    assert support.EXPECTED_DATE_SHA256 == "ed5e64f0fdf3fcb968462521f307c545f74b0cc00c006e618f9bc8a1c1931e9f"
    ...
    assert runner.LANDING_ROOT.name == "a007_investor_h2"
    assert support.EXPECTED_BUSINESS_DATA == {
        "bld": support.BUSINESS_BLD, "strtDd": "20120105", "endDd": "20140103",
...
def test_expected_494_dates_are_bound_to_exact_retained_sources():
    ...
    assert dates[0] == "20120105" and dates[-1] == "20140103"
    digest = hashlib.sha256(("\n".join(dates) + "\n").encode()).hexdigest()
    assert digest == support.EXPECTED_DATE_SHA256


def test_h2_classifies_only_exact_full_set_or_exact_end_date_zero_collapse():
    full = support.classify_response(_body(), FIXTURE_DATES)
    assert full.classification == "H2_FULL_RANGE_AVAILABLE"
    collapsed = support.classify_response(_body(("20140103",), total=0), FIXTURE_DATES)
    ...
    with pytest.raises(PilotStopped, match="AMBIGUOUS_STOP:1/3"):
        support.classify_response(_body(("20140103",), total=1), FIXTURE_DATES)
...
@pytest.mark.parametrize("body,reason", [
    ...
    (json.dumps({"OutBlock_1": [{**_row("20140103", 0), "extra": "x"}]}).encode(), "SCHEMA_MISMATCH"),
    (json.dumps({"OutBlock_1": [{**_row("20140103", 0), "TRD_DD": "bad"}]}).encode(), "INVALID_DATE"),
    (json.dumps({"OutBlock_1": [{**_row("20140103", 0), "STR_CONST_VAL1": "-1"}]}).encode(), "NEGATIVE_VALUE"),
    (json.dumps({"OutBlock_1": [{**_row("20140103", 0), "STR_CONST_VAL1": "1"}]}).encode(), "INVESTOR_TOTAL_MISMATCH"),
])
def test_h2_strict_html_schema_date_and_domain_gates(body, reason):
    with pytest.raises(PilotStopped, match=reason):
        support.classify_response(body, FIXTURE_DATES)


def test_h2_cli_refuses_without_all_confirmations_before_network(monkeypatch):
    called = False
    ...
    monkeypatch.setattr("sys.argv", ["diagnose_a007_investor_h2.py"])
