...
from scripts.manual import pilot_pykrx_foreign_ownership as runner
from scripts.manual import pykrx_foreign_ownership_pilot_support as support
...
def test_fixed_matrix_is_full_market_primary_and_symbol_qa_bounded():
    ...
    assert all(item.bld.endswith("03701") for item in support.PROBE_MATRIX[:4])
    assert all(item.bld.endswith("03702") for item in support.PROBE_MATRIX[4:])
    assert [item.scope["date"] for item in support.PROBE_MATRIX[2:4]] == ["20080102", "20080102"]
    assert support.PROBE_MATRIX[5].scope["symbol"] == "003410"
    assert support.PROBE_MATRIX[6].scope["symbol"] == "030270"
...
def test_retained_c005_raw_symbol_semantics_and_ratio_checks():
    ...
    classification, rows, audit = support.classify_business_body(support.PROBE_MATRIX[4], body)
...
def test_valid_empty_distinct_from_failure_and_html():
    historical = support.PROBE_MATRIX[5]
