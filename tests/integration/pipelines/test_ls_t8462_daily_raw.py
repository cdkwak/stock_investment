...
import scripts.manual.collect.collect_ls_t8462_daily_raw as daily
from scripts.manual.collect.collect_ls_t8462_daily_raw import (
    INSTITUTION_FIELDS, daily_scopes, institution_reconciliation,
    retention_transition, validate_market_date,
)
from scripts.manual.backfill.ls_derivatives_raw_backfill import EXPECTED_ROW_KEYS
...
def test_failed_attempt_does_not_consume_the_date_but_success_does(tmp_path):
    root = tmp_path / "data/landing/ls_openapi/t8462_daily_raw/20260818"
    failed = root / "failed/checkpoint.json"
    failed.parent.mkdir(parents=True)
    failed.write_text(json.dumps({
        "status": "DAILY_COLLECTION_STOPPED", "secret_scan": "PASS",
    }), encoding="utf-8")
    assert daily._successful_attempt_exists(tmp_path, "20260818") is False

    success = root / "success/checkpoint.json"
    success.parent.mkdir(parents=True)
    success.write_text(json.dumps({
        "status": "DAILY_COLLECTION_COMPLETE", "secret_scan": "PASS",
    }), encoding="utf-8")
    assert daily._successful_attempt_exists(tmp_path, "20260818") is True
...
def test_live_shape_is_18_calls_durable_and_same_date_rerun_is_pre_network(tmp_path, monkeypatch):
    ...
    semantic_statuses = [
        json.loads(path.read_text(encoding="utf-8"))["semantic_status"]
        for path in run_dir.glob("*.provenance.json")
    ]
    assert all(item["U"] == "CONFIRMED_EMPIRICAL_ALL" for item in semantic_statuses)
    assert all(
        item["D"] == "CONFIRMED_EMPIRICAL_REGULAR_WITH_CATEGORY_BOUNDARY"
        and item["N"] == "CONFIRMED_EMPIRICAL_NIGHT_WITH_CATEGORY_BOUNDARY"
        for item in semantic_statuses
    )
    assert all(item["session_finality"] == "UNRESOLVED" for item in semantic_statuses)
    assert all(item["predictive_pit"] == "BLOCKED" for item in semantic_statuses)
    assert all(item["option_u_historical_difference"] == "AGGREGATE_FIELD_SEMANTICS" for item in semantic_statuses)
